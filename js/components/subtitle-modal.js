    ...
    <div id="subtitle-modal" class="fixed inset-0 z-[110] bg-[#0f172a] transform translate-y-full transition-transform duration-300 flex flex-col">
        <div class="flex justify-between items-center px-4 sm:px-6 py-4 border-b border-white/10 shrink-0 bg-[#0f172a]">
            ...
            <div class="flex gap-2">
                <button id="btn-close-sub-modal" class="px-5 py-2.5 bg-slate-700/80 hover:bg-slate-600 text-sm font-semibold rounded-xl transition-colors text-white shadow-md" data-i18n="subtitleModal.btnClose">${t('subtitleModal.btnClose')}</button>
            </div>
        </div>
        <div class="flex-grow flex flex-col gap-6 overflow-hidden w-full bg-[#0f172a]">
            ...
            <div class="flex justify-center gap-4 shrink-0 pt-4 sm:pt-6 px-4 sm:px-6">
                <label class="w-14 h-14 bg-blue-600 hover:bg-blue-500 rounded-2xl cursor-pointer shadow-lg flex items-center justify-center transition-transform hover:scale-105" data-i18n-title="subtitleModal.btnUpload.title" title="${t('subtitleModal.btnUpload.title')}">
                ...
                <button id="btn-auto-timing" class="w-14 h-14 bg-rose-600 hover:bg-rose-500 rounded-2xl shadow-lg flex items-center justify-center transition-colors relative" data-i18n-title="subtitleModal.btnAutoTiming.title" title="${t('subtitleModal.btnAutoTiming.title')}">
                ...
                <button id="btn-add-sub" class="w-14 h-14 bg-indigo-500 hover:bg-indigo-400 rounded-2xl shadow-lg flex items-center justify-center transition-transform hover:scale-105" data-i18n-title="subtitleModal.btnAddSub.title" title="${t('subtitleModal.btnAddSub.title')}">
                ...
                <button id="btn-apply-sub" class="w-14 h-14 bg-emerald-500 hover:bg-emerald-400 rounded-2xl shadow-lg flex items-center justify-center transition-transform hover:scale-105" data-i18n-title="subtitleModal.btnApplySub.title" title="${t('subtitleModal.btnApplySub.title')}">
            ...
            <div class="text-xs font-bold text-slate-400 uppercase flex justify-between items-center tracking-wider border-b border-white/5 pb-2 px-5 shrink-0">
                <span data-i18n="subtitleModal.listHeading">${t('subtitleModal.listHeading')}</span>
                <button id="btn-export-srt" class="text-sky-400 hover:text-sky-300 underline font-semibold normal-case" data-i18n="subtitleModal.btnExportSrt">${t('subtitleModal.btnExportSrt')}</button>
            </div>
